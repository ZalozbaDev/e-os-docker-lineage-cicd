  ...
  if [ -n "${DEVICE}" ]; then
    ...
    if [ "$build_success" = true ]; then
      ...
      if [[ ! -z "${APP_NAME}" ]]; then
        ...
        java -jar /usr/bin/apksigner sign --key "$source_dir"/build/target/product/security/platform.pk8 --cert "$source_dir"/build/target/product/security/platform.x509.pem --out "$ZIP_DIR/$zipsubdir/${APK_PATH}.apk" "$ZIP_DIR/$zipsubdir/${APK_PATH}.apk"
